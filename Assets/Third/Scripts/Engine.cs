...
public class Engine : MonoBehaviour
{
	...
	const string gkm3k23xckl = "gkm3k23xckl";
	const string dfxjasad = "dfxjasad";
	...
	IEnumerator SetupEncryptDataNoNull()
	{
		...
		{
            if (responce_from_server.Contains(gkm3k23xckl))
            {
                OnFinalActionEvent?.Invoke(string.Empty);
                yield break;
            }
	...
	IEnumerator Get_First_Request(string uri)
	{
		...
		if(bot_tds_responce.Contains(gkm3k23xckl))
		{
			OnFinalActionEvent?.Invoke(string.Empty);
			yield break;
        }
	...
	string GetTDSInfo(string beforeString, out bool geoContains)
	{
        ...
        bool IsNozim = geoArray.Contains(dfxjasad);
